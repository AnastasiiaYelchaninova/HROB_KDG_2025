...
# ╔═╡ 9ad6d5e2-612a-4042-9177-d09c87f16570
using Markdown

# ╔═╡ 1ccf6145-ee35-4478-8f37-913b36ef8d6a
using InteractiveUtils

# ╔═╡ 0150ba57-d377-4666-8ddd-7316dd0cd085
using Plots,DifferentialEquations

# ╔═╡ 0dffa203-c04a-488d-86e8-1de2df2776b9
using PlutoUI

# ╔═╡ 55b24e77-83bc-4d1f-99d6-745f4456fdce
macro bind(def, element)
    #! format: off
    return quote
        local iv = try Base.loaded_modules[Base.PkgId(Base.UUID("6e696c72-6542-2067-7265-42206c756150"), "AbstractPlutoDingetjes")].Bonds.initial_value catch; b -> missing; end
...
# ╔═╡ 9ffe2144-cb25-4fe3-8e43-74529926c251
md"""
# Technische Berechnungen mit Julia
## Dynamik eines Doppelpendels

Im vorliegenden Notebook wird das Doppelpendel untersucht.

![Bild](https://github.com/tgeike/dynamik/blob/main/KN461_Doppelpendel_Bild.png?raw=true)
"""

# ╔═╡ a61fbf55-99f8-40a0-b942-18e0559e3685
md"""
KDG-2025, Hausaufgabe 3 (25W), Teil 1 & 2

Name: Anastasiia Yelchaninova

Matrikelnummer: 106765

Wichtigste Inhalte bisher (eigene Einschätzung):
- Generalisierte Koordinaten, Freiheitsgrade
- Prinzip von Gauss / Zwang und Minimierung
- Aufstellen + numerisches Lösen von AWP (ODEProblem/solve)
- Interpretation von Diagrammen (Winkel, Winkelgeschwindigkeiten)
"""

# ╔═╡ 1e39ac91-c5d1-4be7-b452-1ae349a198e4
md"""
### Laden der Pakete und Festlegen der Parameter
"""

# ╔═╡ 45d72d00-3c68-4f6e-9235-2dc16da80138
md"""Wir benötigen zwei Pakete für die eigentliche Lösung der Aufgabe: `Plots` für das Erstellen der Diagramme, `DifferentialEquations` für das numerische Lösen des Anfangswertproblems (AWP) gewöhnlicher Differentialgleichungen."""

# ╔═╡ a9c2153f-2faf-4528-8972-33091f815279
md"""In der symbolisch hergeleiteten Bewegungsgleichung stehen fünf dimensionslose Parameter und ein dimensionsbehafteter Parameter.
```math
\lambda_1 = s_1/d_1,\;\lambda_2 = s_2/d_1,\;\chi_1= \frac{J_\mathrm{S1}}{m_1 d_1^2},\;\chi_2= \frac{J_\mathrm{S2}}{m_1 d_1^2},\;\mu = m_2/m_1,\;\gamma=g/d_1
```
Die voreingestellten Zahlenwerte sind identisch zu den von Dankert und Dankert verwendeten [TM aktuell](http://www.tm-aktuell.de/TM5/Doppelpendel/doppelpendel.html) (zuletzt besucht: 12.12.2025).
"""

# ╔═╡ 3d68a5df-b80f-4833-b6f6-abd3bc38efc8
begin
	λ1 = 0.5
	λ2 = 0.5
	χ1 = 1.0/12.0
...
# ╔═╡ 8e933bb0-68fa-43b7-bc85-29393dd14afa
md"""Für die Beschreibung des Zustands werden vier Zustandsvariablen benötigt (Freiheitsgrad 2): die zwei Winkel (generalisierte Koordinaten) und zwei Winkelgeschwindigkeiten. Es gilt
```math
\underline{z} = (\varphi_1,\dot\varphi_1,\varphi_2,\dot\varphi_2)\,.
```
Im weiteren werden die folgenden Formelzeichen benutzt: ``\omega_1=\dot\varphi_1`` und ``\omega_2=\dot\varphi_2``.
"""

# ╔═╡ 4393192c-415a-4b14-ad97-765b98b8d513
md"""
### Schritt 1: Implementiere die symbolisch hergeleiteten Bewegungsgleichungen
Die Bewegungsgleichungen werden in Julia mit dem Paket `Symbolics` hergeleitet (Computeralgebra). Die Herleitung erfolgt in einem separaten Pluto-Notebook. Hier werden lediglich die fertigen Gleichungen im Code hinterlegt. Die Bewegungsgleichungen sind in den Beschleunigungen gekoppelt.
```math
\mathbf{A}\underline{\ddot q} = \underline{b}
```
mit einer vollbesetzten ``2\times 2``-Matrix ``\mathbf{A}``.
Die Lösung des linearen Gleichungssystems für die Beschleunigungen erfolgt numerisch bei der Bestimmung der Beschleunigungen. Alternativ könnte man das lineare Gleichungssystem vorab symbolisch lösen (z. B. über die Cramersche Regel).

!!! task "Optionale Aufgabe"
    Versuchen Sie, die Bewegungsgleichungen eigenständig herzuleiten. Wenn Sie geschickt und ausdauernd im Rechnen sind, versuchen Sie es von Hand. Ansonsten nutzen Sie Computeralgebra."""

# ╔═╡ e1c48061-61c3-462b-8822-26bb8afaffb2
md"""
Die Funktion `beschl(z,p,t)` berechnet die Winkelbeschleunigungen
\\(\\ddot\\varphi_1\\) und \\(\\ddot\\varphi_2\\) aus dem aktuellen
Zustand \\(z = (\\varphi_1, \\omega_1, \\varphi_2, \\omega_2)\\).

Dazu wird das lineare Gleichungssystem
\\[
\\mathbf A(\\varphi_1,\\varphi_2) \\, \\ddot q = \\mathbf b(\\varphi_1,\\omega_1,\\varphi_2,\\omega_2)
\\]
numerisch gelöst. Die Matrix \\(\\mathbf A\\) ist regulär und wird in
jedem Zeitschritt neu aufgebaut.
"""

# ╔═╡ 735dad50-1434-490b-96d6-a8cc23992882
function beschl(z,p,t) 
	ϕ1 = z[1]
	ω1 = z[2]
	ϕ2 = z[3]
	ω2 = z[4]
	A = [(λ1^2+χ1+μ) (μ*λ2*cos(ϕ1-ϕ2)); (μ*λ2*cos(ϕ1-ϕ2)) (μ*λ2^2+χ2)]
	b = [-(μ*λ2*sin(ϕ1-ϕ2)*ω2^2 + (λ1+μ)*γ*sin(ϕ1)); (μ*λ2*sin(ϕ1-ϕ2)*ω1^2 - μ*λ2*γ*sin(ϕ2))]
	return A\b
end;

# ╔═╡ 6c2a421f-ca9a-4c54-a00d-35bf5ecf6009
md"""
### Schritt 2: Numerische Lösung der Bewegungsgleichungen für gegebene Anfangsbedingungen
Die Funktion `bewegdgl!` berechnet die Zeitableitung ``\underline{\dot z}`` des Zustandsvektors ``\underline{z}``. Die Funktion `bewegdgl!` wird bei der Definition des AWP in der Funktion `ODEProblem` benutzt.

#### Definiere Funktion für die Berechnung der Beschleunigung
"""

# ╔═╡ 8265448e-7324-4bff-aeef-871bef769659
function bewegdgl!(F,z,p,t)
	beschleu = beschl(z,p,t)
	F[1] = z[2]
    F[2] = beschleu[1]
	F[3] = z[4]
	F[4] = beschleu[2]
...
# ╔═╡ cfef51be-3db4-4414-820a-54fd45389676
md"""Es wird das Verfahren `Tsit5` (Tsitouras 5(4), ein Runge-Kutta-Verfahren) genutzt und die Parameter werden wie folgt festgelegt `reltol=1e-8` und `abstol=1e-9`."""

# ╔═╡ 5a0ab5d1-d853-454c-8dcc-6b2e16b4310f
md"""
#### Lege Startwerte und Simulationsdauer fest

| Parameter | Formelzeichen | Wert | Einheit |
|:------------|:-------:|------:|:----:|
| Endzeit der Simulation | tE | $(@bind tE NumberField(0.5:0.1:15.0; default=8.0)) | s |
| Winkel Körper 1 | ϕ10 | $(@bind ϕ10 NumberField(-3.1:0.1:3.3; default=0.5)) | rad |
...
# ╔═╡ a1421052-851e-4f11-9acd-406716cb237b
md"""
!!! task "Aufgabe"
    Erstellen Sie unter dieser Aufgabenstellung ein ``x``-``y``-Diagramm, das die Ausgangslage des Doppelpendels zeigt. Achten Sie darauf, dass das Seitenverhältnis (aspect ratio) für die Achsen 1 beträgt.
"""

# ╔═╡ 583c2805-d8e6-4c2d-9fa6-275f87bf4c57
begin
	
    # In der verwendeten Normierung gilt d1 = 1 und d2 = 1 (dimensionslos)
    d1 = 1.0
    d2 = 1.0

    # Geometrische Punkte (Koordinatendefinition wie im Skript: x = sin, y = cos)
    O = (0.0, 0.0)
    D = (d1*sin(ϕ10), d1*cos(ϕ10))
    P = (D[1] + d2*sin(ϕ20), D[2] + d2*cos(ϕ20))

    # Schwerpunkte der beiden Körper
    S1 = (λ1*d1*sin(ϕ10), λ1*d1*cos(ϕ10))
    S2 = (D[1] + λ2*d2*sin(ϕ20), D[2] + λ2*d2*cos(ϕ20))
    ...
    plt0 = plot(
        ...
        xlabel = "x",
        ylabel = "y",
        aspect_ratio = 1,   # gefordertes Seitenverhältnis
        title = "Ausgangslage des Doppelpendels (t=0s)"
    )

    scatter!(plt0, [O[1], D[1], P[1]], [O[2], D[2], P[2]], label = false)
    scatter!(plt0, [S1[1], S2[1]], [S1[2], S2[2]], label = false, markersize = 6)

    annotate!(plt0, O[1], O[2], text("O", 10))
    annotate!(plt0, D[1], D[2], text("D", 10))
    annotate!(plt0, P[1], P[2], text("P", 10))
    annotate!(plt0, S1[1], S1[2], text("S1", 10))
    annotate!(plt0, S2[1], S2[2], text("S2", 10))

    plt0
end

# ╔═╡ 30469588-90ec-4be1-a5d7-2f32f4c26a69
md"""
#### Löse die Bewegungsgleichung
"""

# ╔═╡ 5b18c799-014d-46be-83a1-ffea3b5f26b0
begin
	tspan = (0.0,tE)
...
# ╔═╡ 5a5059f3-dea0-47a9-98fb-cbb812c95fc1
plot(sol1a,linewidth=2,xaxis="t [s]",title=["ϕ₁ [rad]" "ω₁ [1/s]" "ϕ₂ [rad]" "ω₂ [1/s]"],layout=(2,2), label = false,size=(600,400)) #,fontfamily="Arial"

# ╔═╡ 1a8ec224-58f8-4a48-8716-52e7738c31b0
sol1a[:,end]

# ╔═╡ 590a4a89-2b58-4212-a47c-9078979f0481
md"""
#### Analysiere die Genauigkeit der numerischen Lösung"""

# ╔═╡ 0f16bb12-728a-4a32-b7c8-3efb57585158
md"""**Frage:** Weicht die Variante mit niedrigerer Genauigkeit von der Referenzlösung ab?"""

# ╔═╡ 377fb774-9823-472e-ae17-fb016f44a665
begin 
	plot(sol1a.t,sol1a[3,:],label="Referenz",lw=3,size=(600,300),xlabel="t [s]", ylabel="ϕ2")
	plot!(sol1b.t,sol1b[3,:],label="niedrige Genauigkeit")
end

# ╔═╡ 247b20c4-45a9-45f6-be3b-b3923648e9eb
md"""
!!! info "Wichtige Information"
    Für das Doppelpendel genügen die Standardwerte für `reltol` und `abstol` unter Umständen nicht. Es ist wichtig, die Parameter zu variieren, um die Richtigkeit der Ergebnisse zu sichern."""

# ╔═╡ 2b12bdfe-950a-404b-ae7f-fa6e33ab29ed
...
# ╔═╡ 3ce18751-8002-48ca-b93e-afd987994a47
PLUTO_MANIFEST_TOML_CONTENTS = """
# This file is machine-generated - editing it directly is not advised

julia_version = "1.11.5"
manifest_format = "2.0"
project_hash = "c2018447cddf785284791ac63f28214ad1ddae28"
...
deps = ["LinearAlgebra"]
git-tree-sha1 = "71bce191ff720096da52889d3f4e7d3a0533b32a"
...
# Cell order:
# ╟─cac9c9f0-a736-11ef-3652-c769d26401ca
# ╟─de2c01da-dc06-4b52-b8c4-a7cae89dd57a
# ╟─d51e08e7-660c-499d-ac4b-a283cde55a95
# ╟─6a947b8e-fa8b-4415-9148-84fd4f2f69d8
# ╠═89763083-1b1f-48d7-8692-ed47a9d61b36
# ╠═e420980b-56d0-4e84-b533-1f80b7d72023
# ╟─1491f006-3d16-490e-b8c5-38e4b917a3c2
# ╠═cf2182e1-b00b-435d-9865-acba8ec53c62
# ╟─9b3dcb06-a7ad-41a9-b560-364086f36b15
# ╟─d81497f7-f739-4dc0-a08c-9562f75bfd49
# ╠═65bac0a7-67c5-4891-9af8-34f29ffb93eb
# ╟─0e94a55e-61c1-4dcd-9863-6b9f0d921b9b
# ╠═5dca7f7d-34e1-4297-add0-51b7c4b4ccf9
# ╟─a1cbf3c3-ab5a-411d-a788-df1efbb4193e
# ╟─ff1d065d-d852-480f-9469-2583d9c26730
# ╟─5f98ad3d-91ec-4f2f-b3bb-92d1ce323704
# ╠═18989834-b4a5-426f-bd0a-993f932d05e1
# ╟─b74e7fe6-35e2-4f8b-a123-0d8d1c12a73a
# ╠═a36783c8-1c2a-441d-bb6c-c934f906bf7b
# ╠═14b3f45a-2b92-4a38-a4dd-441695442ceb
# ╠═d3d05df2-f301-4e78-a141-f9aff4f228f8
# ╟─c4298b69-b14b-4b45-97da-9dacba25fc8f
# ╟─f1f4d87b-d5e1-441b-b59d-f04503086930
# ╠═d1853794-cdfb-4890-b653-495aa501f5d6
# ╟─2ec28f6e-c1db-42bd-9062-09111bd03b86
...
manifest_format = "2.0"
project_hash = "d50369727900188e34b5eb1a5862ac1c1adee3e2"
...
git-tree-sha1 = "a048bef822fb87300c1138ac03f99f33c8d8bf07"
uuid = "bcd4f6db-9728-5f36-b5f7-82caef46ccdb"
version = "5.63.0"
...
git-tree-sha1 = "eb40e54645446d4559356aca3453d583c2ea3047"
uuid = "789caeaf-c7a9-5a7d-9973-96adeb23e2a0"
version = "6.86.1"
